const dict = {
  en: {
    ...
    'fn.1.btn': 'Open',
    'fn.1.info': 'A basic button runs an action on click. Use it to submit, confirm, or open a window.',
    'fn.2.title': '2. Textured button',
    'fn.2.btn': 'Open',
    'fn.2.info': 'This button changes its look on hover and while it is held down, so each state is easy to see. Use it when a control should feel tactile.',
    'fn.3.title': '3. On / Off',
    'fn.3.off': 'Off',
    'fn.3.on': 'On',
    ...
    'fn.4.info': 'Expandable text keeps extra lines hidden until the field is clicked. Use it for FAQs and long notes so the page stays short.',
    ...
    'fn.6.btn': 'Show',
    'fn.6.info': 'The scrollbar can change color, width, and corner shape. That is how it is matched to the page. Here the thumb uses the hover color, is twice as wide as usual, and has sharp corners.',
    'fn.6.track': 'The track behind the thumb is empty — only the thumb is filled.',
    'fn.6.more': 'More lines so the bar appears and you can drag it.',
    'fn.7.title': '7. Search',
    ...
    'fn.7.item3': 'Each note is checked against the query; notes that do not match are hidden.',
  ...
  ru: {
    ...
    'fn.1.btn': 'Открыть',
    'fn.1.info': 'Обычная кнопка запускает действие по нажатию. Её ставят на отправку, подтверждение или открытие окна.',
    'fn.2.title': '2. Кнопка с текстурами',
    'fn.2.btn': 'Открыть',
    'fn.2.info': 'Эта кнопка меняет вид при наведении и пока её держат нажатой, чтобы каждое состояние было видно. Так делают, когда элемент должен ощущаться нажимаемым.',
    ...
    'fn.4.info': 'Развёртываемый текст прячет лишние строки, пока по полю не нажмут. Его ставят в FAQ и длинные заметки, чтобы страница оставалась короткой.',
    ...
    'fn.6.btn': 'Показать',
    'fn.6.info': 'У полосы прокрутки можно менять цвет, ширину и форму краёв. Так её подгоняют под стиль страницы. Здесь бегунок цвета наведения, вдвое шире обычного, с острыми углами.',
    'fn.6.track': 'Дорожка за бегунком пустая — залит только сам бегунок.',
    'fn.6.more': 'Ещё строки, чтобы полоса появилась и её можно было подвигать.',
    'fn.7.title': '7. Поиск',
    ...
    'fn.7.item3': 'Запрос сравнивается с каждой заметкой; те, что не совпали, скрываются.',
